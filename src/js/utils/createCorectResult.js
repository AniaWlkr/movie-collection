...
export default function createCorectResult(results) {
  ...
  for (let j = 0; j < results.length; j++) {
    ...
    try {
      for (let i = 0; i < result.genre_ids.length; i++) {
        ...
        let genre = genresArr.find(genre => genre.id === result.genre_ids[i]);
        if (genre) {
  ...
  //формуєм результати віддаєм у потрібному промісі
  const res = new Promise(resolve => resolve(results));
  return res;
}
